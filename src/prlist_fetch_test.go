...
func TestFetchPRSupplementalBatchRecoversHealthyAliasesFromPartialError(t *testing.T) {
	...
	ghExecFunc = func(args ...string) (bytes.Buffer, bytes.Buffer, error) {
		...
		if !strings.Contains(query, "approvedReviews: reviews(states: [APPROVED], last: 100) { nodes { submittedAt author") {
			t.Fatalf("supplemental query must request approval timestamps, got %q", args)
		}
